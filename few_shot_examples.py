...
# 샘플 수정사항 패턴
SAMPLE_EXAMPLES = """
### 학습 예시 1: 기본 박스 + 화살표 수정 패턴 (가장 중요!) ⚠️⚠️⚠️
...
⚠️ 핵심 규칙:
- **박스 안의 텍스트 = original_text (대상문구)**
- **화살표 끝의 텍스트 = new_text (변환문구)**
- 취소선이나 물결선이 없으면 = "수정"
...
### 학습 예시 4-1: 다양한 박스 형태 + 화살표 패턴 (매우 중요!) ⚠️⚠️⚠️
**박스 + 화살표 패턴은 다양한 형태로 나타납니다. 모두 "수정" 지시사항입니다!**

#### 패턴 A: 직사각형 박스 + 위쪽 화살표 (가장 일반적)
이미지 설명:
  - 붉은색/주황색 직사각형 박스로 텍스트를 둘러쌈
  - 박스 위쪽에서 화살표가 위로 향함
  - 화살표 끝에 붉은색 손글씨로 새 텍스트
  - 완전한 사각형 형태
→ **100% 수정 지시사항!**

#### 패턴 B: 직사각형 박스 + 아래쪽 화살표
이미지 설명:
  - 붉은색 직사각형 박스로 텍스트를 둘러쌈
  - 박스 아래쪽 또는 오른쪽에서 화살표가 아래로 향함
  - 화살표 끝에 붉은색 손글씨로 새 텍스트
→ **100% 수정 지시사항!**

#### 패턴 C: 타원형/원형 박스 + 화살표
이미지 설명:
  - 붉은색/분홍색 타원형 또는 원형으로 텍스트를 둘러쌈
  - 타원 위쪽에서 화살표가 위로 향함
  - 화살표는 곡선 형태일 수 있음
  - 화살표 끝에 손글씨로 새 텍스트
→ **100% 수정 지시사항!**
...
#### 패턴 D: 불규칙한 박스 + 화살표
이미지 설명:
  - 손으로 그린 듯한 불규칙한 박스 (직사각형이지만 선이 삐뚤삐뚤)
  - 어느 방향으로든 화살표가 나감 (위, 아래, 오른쪽, 왼쪽)
  - 화살표는 직선 또는 곡선
  - 화살표 끝에 손글씨로 새 텍스트
→ **100% 수정 지시사항!**
...
#### 패턴 F: 박스 + 곡선 화살표
이미지 설명:
  - 어떤 형태의 박스든 (직사각형, 타원형, 불규칙)
  - 화살표가 곡선으로 크게 휘어져서 새 텍스트를 가리킴
  - 화살표 끝에 손글씨 또는 타이핑된 텍스트
→ **100% 수정 지시사항!**

⚠️⚠️⚠️ **핵심 규칙**:
- **박스 형태에 관계없이 (직사각형, 타원형, 원형, 불규칙) = 수정!**
- **화살표 방향에 관계없이 (위, 아래, 왼쪽, 오른쪽, 곡선) = 수정!**
- **색상에 관계없이 (붉은색, 주황색, 분홍색, 진한 빨강) = 수정!**
- **선 굵기에 관계없이 (얇은 선, 두꺼운 선) = 수정!**
- **박스가 완전하지 않아도 (한쪽이 열려있어도) 화살표가 있으면 = 수정!**
- **박스 + 화살표 = 무조건 action = "수정"!**
- **화살표 끝의 텍스트 = new_text (변환문구)!**
...
### 학습 예시 8-1: 화살표 끝 텍스트의 두 가지 유형 (매우 중요!) ⚠️⚠️⚠️

#### 유형 1: 붉은색 손글씨 (가장 일반적)
이미지 설명:
  - 박스 안: 원본 텍스트 (검은색 인쇄)
  - 화살표로 연결
  - **화살표 끝: 붉은색 계열 손글씨로 새 텍스트**
  - 손글씨는 필기체 또는 인쇄체 스타일
  - 색상: 붉은색, 주황색, 분홍색, 진한 빨강
예시:
  - 박스 안: "KHALIFA PORT, ABU DHABI, UAE"
  - 화살표 끝 손글씨: "KHALIFA PORT, U.A.E"
→ **new_text = "KHALIFA PORT, U.A.E"**

#### 유형 2: 붉은색 타이핑된 텍스트 (중요!)
이미지 설명:
  - 박스 안: 원본 텍스트 (검은색 인쇄)
  - 화살표로 연결
  - **화살표 끝: 붉은색으로 타이핑된 텍스트 (인쇄체)**
  - 손글씨가 아닌 컴퓨터로 타이핑한 텍스트
  - 색상: 붉은색 계열
  - 간혹 또 다른 붉은색 박스 안에 있을 수 있음
예시:
  - 박스 안: "CHISAGE RESOURCE GROUP CO., LTD..."
  - 화살표 끝 타이핑 텍스트: "CHISAGE RESOURCE GROUP CO.,LTD\\nCHISAGE BUILDING 10F..."
→ **new_text = 붉은색 타이핑된 모든 텍스트**

⚠️⚠️⚠️ **핵심 규칙**:
- **화살표 끝에 손글씨든 타이핑된 텍스트든 = 모두 new_text!**
- **붉은색 계열이면 손글씨가 아니어도 수정 지시사항!**
- **타이핑된 텍스트가 또 다른 박스 안에 있어도 = new_text!**
- **화살표가 가리키는 모든 붉은색 텍스트를 읽으세요!**
- **여러 줄이면 \\n으로 구분!**
"""
...
### 1. 수정(Modification) 패턴의 특징 - 박스 + 화살표 (가장 중요!) ⚠️⚠️⚠️:

#### 핵심 구조: **박스 + 화살표 = 100% 수정!**
✓ **박스 안 = 수정이 필요한 원본 텍스트 (original_text)**
✓ **화살표 = 연결 표시**
✓ **화살표 끝 = 변경할 새 텍스트 (new_text)**

#### 박스 형태 (모든 형태 인식):
✓ 직사각형 박스 (완전한 사각형, 모서리 둥근 형태)
✓ 타원형/원형 박스
✓ 불규칙한 손그림 박스 (삐뚤삐뚤한 선)
✓ 한쪽이 열린 박스 (완전하지 않아도 OK)
...
#### 화살표 끝의 새 텍스트 유형:
✓ **붉은색 계열 손글씨** (가장 일반적)
✓ **붉은색 계열 타이핑된 텍스트** (인쇄체, 컴퓨터 타이핑)
✓ 간혹 또 다른 붉은색 박스 안에 타이핑된 텍스트
...
### 2-2. 박스 + 화살표 패턴 인식 규칙 (매우 중요!) ⚠️⚠️⚠️

#### 기본 규칙: **박스 + 화살표 = 무조건 "수정"!**
✓ **박스 형태에 관계없이** (직사각형, 타원형, 원형, 불규칙)
✓ **화살표 방향에 관계없이** (위, 아래, 왼쪽, 오른쪽, 곡선)
✓ **색상에 관계없이** (붉은색, 주황색, 분홍색, 진한 빨강)
✓ **선 굵기에 관계없이** (얇은 선, 두꺼운 선, 중간 굵기)
✓ **박스가 완전하지 않아도** (한쪽이 열려있거나 불완전해도)
→ **박스 + 화살표가 보이면 = 100% "수정" 지시사항!**

#### 화살표 끝 텍스트 읽기 규칙:
✓ **붉은색 손글씨**: 가장 일반적, 필기체 또는 인쇄체 스타일
✓ **붉은색 타이핑된 텍스트**: 컴퓨터로 타이핑한 인쇄체
✓ **또 다른 박스 안의 타이핑 텍스트**: 붉은색 박스 안에 타이핑된 텍스트
✓ **여러 줄의 주소, 전화번호, 이메일 등**: 모든 줄을 읽고 \\n으로 구분
→ **화살표가 가리키는 모든 붉은색 텍스트 = new_text (변환문구)!**

#### 박스 안 텍스트 읽기 규칙:
✓ **박스 안의 검은색 인쇄된 텍스트 = original_text (대상문구)**
✓ **박스가 여러 줄을 포함하면 모든 줄을 읽고 \\n으로 구분**
✓ **한 글자도 빠뜨리지 말고 정확히 읽기**
...
##### 화살표 끝 텍스트 유형:
- ✅ **붉은색 손글씨** (가장 일반적)
- ✅ **붉은색 타이핑된 텍스트** (인쇄체, 컴퓨터 타이핑)
- ✅ **또 다른 박스 안의 타이핑 텍스트**

→ **박스 + 화살표가 보이면 취소선 없으면 무조건 "수정"!**

#### 3순위: 기타 마킹
- **번호 표시: #1, #2, #20, #15 = 수정 지시사항 (취소선 없을 때)**
- **박스, 밑줄, 동그라미 = 수정 가능성 높음 (취소선 없을 때)**
