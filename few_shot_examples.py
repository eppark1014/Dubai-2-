...
### 학습 예시 4-1: 박스 + 화살표 패턴 (매우 중요!) ⚠️⚠️⚠️
**박스 + 화살표 = 무조건 "수정" 지시사항입니다!**

이미지 설명: 텍스트가 박스로 표시되고, 화살표가 새 텍스트를 가리킴
  - 원본: 박스 안의 텍스트 (예: "ABU DHABI, U.A.EMIRATES")
  - 화살표: 박스에서 나와서 새 텍스트를 가리킴 (→, ⇒, 곡선 등)
  - 새 텍스트: 화살표 끝의 손글씨 또는 타이핑 (예: "ABU DHABI, U.A.E")
...
⚠️⚠️⚠️ **핵심 규칙**:
- **박스 형태 무관**: 직사각형, 타원형, 원형, 불규칙 모두 OK
- **화살표 방향 무관**: 위, 아래, 왼쪽, 오른쪽, 곡선 모두 OK
- **색상 무관**: 빨강, 주황, 분홍, 진한 빨강 모두 OK
- **선 굵기 무관**: 얇은 선, 두꺼운 선 모두 OK
- **박스가 불완전해도** 화살표만 있으면 = 수정!
- **화살표 끝 텍스트 종류 무관**: 손글씨, 타이핑 모두 OK
...
### 학습 예시 5-1: 회색/검은색 취소선 삭제 패턴 (매우 흔함!) ⚠️⚠️⚠️
이미지 설명: NOTIFY PARTY 섹션의 여러 줄 연락처 정보에 **검은색/회색 취소선**이 그어짐
  - "USCI:91330212MA28040CY4D" - 검은색 취소선
  - "CONTACT PERSON NAME: MS. JOY SHAO" - 검은색 취소선
  - "CONTACT PERSON TEL:86-574-88291892" - 검은색 취소선
  - "EMAIL:JOYSHAO@CHISAGE.COM" - 검은색 취소선
  
⚠️⚠️⚠️ **핵심**: 4개 줄이 모두 세로로 가깝게 배치 → **하나의 항목으로 병합!**

분석 결과:
{
  "order": 5,
  "action": "삭제",
  "original_text": "USCI:91330212MA28040CY4D\\nCONTACT PERSON NAME: MS. JOY SHAO\\nCONTACT PERSON TEL:86-574-88291892\\nEMAIL:JOYSHAO@CHISAGE.COM",
  "new_text": "(DELETE)",
  "location": "Notify Party",
  "confidence": "high"
}
...
⚠️ 중요:
- **검은색/회색 취소선도 빨간색 취소선과 동일하게 "삭제"를 의미!**
- **색상이 중요한 것이 아니라 "선이 텍스트를 관통"하는 것이 중요!**
- **여러 줄이 세로로 가깝게 배치되고 모두 취소선 → 하나의 항목으로 병합!**
- **4개 줄을 4개 항목으로 분리하지 말고, \\n으로 구분하여 1개 항목으로!**
...
#### 🎯 박스 + 화살표 패턴 (가장 중요!) 🎯
✓ **박스로 원본 텍스트 표시** (직사각형, 타원형, 원형, 불규칙 등 어떤 형태든)
✓ **화살표가 박스에서 나옴** (→, ➔, ⇒, 곡선, 어떤 방향이든)
✓ **화살표 끝에 새 텍스트** (손글씨 또는 타이핑)
✓ **색상 무관** (빨강, 주황, 분홍, 어떤 색이든)
→ **박스 + 화살표 = 무조건 "수정"!**
...
⚠️ **색상 중요!**: 취소선/물결선은 **어떤 색이든** 삭제를 의미합니다!
- 검은색/회색 취소선 = 삭제
- 빨간색/분홍색 취소선 = 삭제
- 파란색 취소선 = 삭제
→ **색상이 아니라 "선이 텍스트를 관통하는가"가 중요!**
